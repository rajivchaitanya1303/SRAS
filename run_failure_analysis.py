...
logger = get_logger("run_failure_analysis")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SRAS failure analysis")
    parser.add_argument("--config", type=str, default="configs/base.yaml")
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="Model checkpoint (default: first in registry)")
...
def main() -> None:
    ...
    logger.info("Failure analysis | checkpoint=%s", checkpoint_path)
    ...
    analyzer = FailureAnalyzer(config.evaluation.failure)
    ...
    logger.info(
        "Failure summary | mean_f1=%.4f | selector_fail=%.3f | generator_fail=%.3f",
        summary.get("overall_mean_f1", 0.0),
        summary.get("selector_failure_rate", 0.0),
        summary.get("generator_failure_rate", 0.0),
    )

    if not args.no_plots:
        plotter = PlotGenerator(output_dir=config.evaluation.figures_dir)
        plotter.plot_failure_breakdown(results)
...
if __name__ == "__main__":
    try:
        ...
        with RunLogger("run_failure_analysis"):
            main()
    except ImportError:
        main()
